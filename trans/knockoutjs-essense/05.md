...
如今在web 开发中，RESTful 服务是最常用的。这是因为**REST**（简称**代表性状态转移**）协议具有一些特点，使其易于在此类应用中使用：
...
有关REST 和 RESTful 服务的更多信息，请参阅[http://en.wikipedia.org/wiki/Representational_state_transfer](http://en.wikipedia.org/wiki/Representational_state_transfer) 。
...
3.  To implement the body of these methods, use jQuery AJAX calls ([http://api.jquery.com/jquery.ajax/](http://api.jquery.com/jquery.ajax/)). Such requests to the server are asynchronous and use a concept called promise ([http://api.jquery.com/promise/](http://api.jquery.com/promise/)). A **promise** is just an object that will contain a value in the future. This value is handled by using a callback function.
    ...
    ![Setting CRUD operations in the resource](graphics/7074OS_05_02.jpg)
...
![Using resources in the view-model](graphics/7074OS_05_03.jpg)
...
![Generating mock data with MockJSON](graphics/7074OS_05_04.jpg)
...
此URI 在 URI 的最后一段中具有产品 ID。这意味着 ID=1 的产品将生成类似于`/products/1`的 URI。ID=2 的产品将生成类似于`/products/2`的 URI。
...
7.  Test how you get a description in the alert box.
    ...
    ![Retrieving a product by ID](graphics/7074OS_05_05.jpg)
...
![Creating a new product](graphics/7074OS_05_06.jpg)
...
4.  Add a button in the catalog template next to the edit button:
    ...
    ![Deleting a product](graphics/7074OS_05_07.jpg)
...
![Sending the order to the server](graphics/7074OS_05_08.jpg)
...
3.  Show the user information in the `finish-order-modal.html` template:
    ...
    ![Extending the customer model](graphics/7074OS_05_09.jpg)
