...
`jqmData()`和`jqmRemoveData()`方法可用于向 jQueryMobile应用程序的元素添加或删除数据属性。它们自动处理自定义名称空间。本食谱向您展示了如何使用这些方法。
...
当父元素上设置了`data-enhance="false"`时，它将被所有子元素继承。为了搜索可以使用手动增强或自定义插件创作的元素，jQueryMobile框架提供了一个名为`jqmEnhanceable()`的过滤方法。这个食谱告诉你如何使用它。
...
此配方中使用的`$.mobile.defaultHomeScroll`属性在 jQueryMobile框架内部用于滚动到页面顶部。此值是使用`$.support.scrollTop`属性从浏览器中获取的。如果该值不是`0`，则框架将其设置为`0`。
