...
使用`$.mobile.loadPage()`方法，您可以将背景中的外部页面加载到DOM 中，并在不影响当前页面的情况下增强其内容。这个食谱告诉你如何做同样的事情。
...
![How it works...](graphics/7225_09_01.jpg)
...
![How it works...](graphics/7225_09_02.jpg)
...
`jqmData()`和`jqmRemoveData()`方法可用于向 jQuery 移动应用程序的元素添加或删除数据属性。它们自动处理自定义名称空间。本食谱向您展示了如何使用这些方法。
...
![How it works...](graphics/7225_09_03.jpg)
...
当父元素上设置了`data-enhance="false"`时，它将被所有子元素继承。为了搜索可以使用手动增强或自定义插件创作的元素，jQuery 移动框架提供了一个名为`jqmEnhanceable()`的过滤方法。这个食谱告诉你如何使用它。
...
![How it works...](graphics/7225_09_04.jpg)
...
![How it works...](graphics/7225_09_07.jpg)

接下来，为锚链的`click`事件添加事件处理程序。调用`dispPath()`函数，a并将`href`属性作为参数传递给它。通过调用锚对象上的 jQuery`attr("href")`方法获取`href`属性。最后，调用`event.preventDefault()`和`event.stopPropagation()`方法来阻止点击事件的默认动作。现在，当您点击**链接 1**时，复合`href`属性的 URL 组件显示如下：

![How it works...](graphics/7225_09_08.jpg)
...
向`main.html`添加两个带有ID`bottombtn`和`topbtn`的按钮。用`id="dispdiv"`创建一个空的`div`标记，并用一些冗长的内容填充它。这里，在`pageinit`事件上使用脚本在`#dispdiv`循环中添加 100 行文本。该页面最初显示如下：

![How it works...](graphics/7225_09_13.jpg)

绑定`#bottombtn`按钮的`click`事件调用`$.mobile.silentScroll`，Y 参数为大值（此处为 2000px）。现在，当您点击**页面底部**按钮时，页面滚动到文档底部的 Y 位置（2000px），如下图所示：

![How it works...](graphics/7225_09_14.jpg)
...
此配方中使用的`$.mobile.defaultHomeScroll`属性在 jQuery 移动框架内部用于滚动到页面顶部。此值是使用`$.support.scrollTop`属性从浏览器中获取的。如果该值不是`0`，则框架将其设置为`0`。
