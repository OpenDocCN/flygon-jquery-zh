...
在 jQuery 移动应用程序中，您可以添加按钮、表单控件和列表作为应用程序的页面内容。页面内容是一个属性为`data-role="content"`的`div`容器。您可以使用框架提供的现成样式和布局来显示数据和格式化数据。您可以在**折叠块**和**折叠套**或**手风琴**中分组显示数据。您可以使用**布局网格**显示多列数据。您还可以在应用程序中使用表单和其他 HTML 元素。
