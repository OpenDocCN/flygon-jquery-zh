...
jQuery 移动框架默认使用CSS 类`activeBtnClass`为处于活动状态的按钮设置主题`b`的样式。`activeBtnClass`类有一个默认字符串值`ui-btn-active`。为了给活动页面（视图中或转换中的页面）设置样式，框架使用 CSS 类`activePageClass`，该类具有默认字符串值`ui`-`page-active`。这个方法向您展示了如何将框架配置为使用自定义类而不是这些默认类。
...
![How it works...](graphics/7225_07_1.jpg)

当您点击打开第 1 页按钮时，按钮的激活状态在按下时显示为绿色阴影，如下图所示：

![How it works...](graphics/7225_07_2.jpg)

接下来，页面`#page1`打开，它也有虚线边框：

![How it works...](graphics/7225_07_3.jpg)

当你点击**返回**按钮时，它也会得到一个绿色的阴影：

![How it works...](graphics/7225_07_4.jpg)
...
只要可能，jQuery Mobile框架就会自动使用 Ajax 处理链接点击和表单提交。这可以使用`$.mobile.ajaxEnabled`属性进行配置，默认情况下，该属性的布尔值为`true`。如果 Ajax 被禁用或不受支持，则使用普通 HTTP 请求并进行完整页面加载。URL 哈希侦听也被禁用。此配方向您展示了如何配置`$.mobile.ajaxEnabled`属性。
...
![How it works...](graphics/7225_07_5.jpg)

点击**打开页面 1**按钮，打开`page1.html`，如下所示。该页面通过 Ajax 加载，框架增强了控件。

![How it works...](graphics/7225_07_6.jpg)

接下来，在`main.html`中将`ajaxEnabled`属性设置为`false`，并重新加载页面。现在打开`page1.html`时，元素没有增强，如下图所示：

![How it works...](graphics/7225_07_7.jpg)

## 还有更多。。。

禁用 Ajax 时，将加载整个页面。在`page1.html`中，由于缺少带有 jQuery 移动框架库链接的`<head>`元素，因此该页面没有任何样式或增强。
...
![How it works...](graphics/7225_07_8.jpg)
...
默认情况下，jQuery移动框架在使用 Ajax 加载页面时使用淡入过渡。使用 Ajax 打开对话框时，默认使用**pop**转换。此配方向您展示了如何为应用程序设置不同的默认转换。
...
jQuery 移动框架自动增强了页面中的控件和标记。要跳过增强标记的某些部分，可以使用`$.mobile.ignoreContentEnabled`配置（默认情况下为`false`。这个食谱告诉你如何做同样的事情。
...
![How it works...](graphics/7225_07_9.jpg)
...
默认情况下，jQuery 移动框架在加载新页面时显示主题为`a`且没有任何文本的旋转动画。如果有错误，页面加载超时，显示错误消息`Error Loading Page`，主题为`e`。此配方向您展示了如何更改和自定义页面加载和错误消息。
...
![How it works...](graphics/7225_07_10.jpg)

错误响应导致显示以下错误消息：

![How it works...](graphics/7225_07_11.jpg)
...
创建`main.html`，并通过在包含对`jquery.mobile.js`的引用之前添加前面的脚本来设置此配置。在脚本中，为在应用程序启动时触发的`mobileinit`事件添加事件处理程序。在这里，使用`$.mobile.ns="my-"`配置设置默认名称空间。添加`#main`和`#dialog`页面。

以下屏幕截图显示了通过 DOM 检查器看到的页面：

![How it works...](graphics/7225_07_12.jpg)
...
当您使用嵌套的`listview`时，jQuery移动框架会以`pagename.html&ui-page=subpageidentifier`的形式生成一个子页面。框架使用子页面 URL 键`(&ui-page)`前的哈希段进行导航。此配方向您展示了如何使用自定义子页面 URL 键。它还向您展示了如何使用`$.mobile.hashListeningEnabled`配置。
...
![How it works...](graphics/7225_07_13.jpg)

点击**子页面项目**、，打开子页面中的嵌套列表。地址栏显示自定义子页面 URL 键`my-page`，如下图所示：

![How it works...](graphics/7225_07_14.jpg)

现在，使用浏览器**返回**按钮返回。地址栏中的 URL 会得到更新，但页面不会更改回上一个屏幕，如以下代码所示：

![How it works...](graphics/7225_07_15.jpg)
...
当你点击一个链接时，导航发生，URL 散列被更新。该框架允许您在支持`history.replaceState`API 的浏览器中将 URL哈希替换为完整路径。此配方向您展示了如何使用`$.mobile.pushStateEnabled`配置进行此操作。它还向您展示了如何使用允许框架自动绑定文档中锚链接点击的`$.mobile.linkBindingEnabled`配置。默认情况下，这两个都是`true`。
...
![How it works...](graphics/7225_07_16.jpg)

当您打开**页面 1**时，URL 地址栏将完整路径追加到`main.html`，如下图所示：

![How it works...](graphics/7225_07_17.jpg)
