...
下一个突出显示的代码区域是 jQuery 检查`EventTarget.addEventListener()`方法是否实际可用于该元素，然后使用它将事件侦听器添加到元素中。在最后突出显示的代码区域中，jQuery 将 observer函数添加到其内部列表中，该列表包含附加到该特定元素的相同事件类型的所有观察者。
