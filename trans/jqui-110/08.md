...
在本例中，我们将使用一个不同的PHP 文件，该文件返回的数据与 autocomplete 期望的格式不同。在将数据传递给小部件之前，我们将使用该函数来请求和处理数据。该示例的上下文将是类似于 Facebook 的消息传递系统的前端，在该系统中，自动补全提示可能的消息接收者，但也可以在他们被选中并添加到`<input>`元素后删除。我们将使用的页面将显示在以下屏幕截图中：
