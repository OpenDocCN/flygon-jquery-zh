...
![Implementing a basic drag](graphics/2209OS_11_01.jpg)
...
| `cursor` | `"auto"` | 指定当指针位于拖动对象的上方时要使用的CSS 光标。 |
...
![Adding the drag handle support](graphics/2209OS_11_02.jpg)
...
![Adding the helper elements](graphics/2209OS_11_03.jpg)
...
![Adding the helper elements](graphics/2209OS_11_04.jpg)
...
![Snapping](graphics/2209OS_11_05.jpg)
...
![Draggable event callbacks](graphics/2209OS_11_06.jpg)
...
![Draggable event callbacks](graphics/2209OS_11_07.jpg)

# Draggable 的方法

可拖动交互助手不公开自己的任何独特方法，只公开常见的API 方法，即`destroy`、`disable`、`enable`、`option`和`widget`。

# 可拖放小部件入门

简而言之，jQueryUI 的 Dropables组件为我们提供了一个放置拖放对象的位置。页面的某个区域被定义为可拖放区域，当拖动对象被拖放到该区域上时，会触发其他内容。使用此组件公开的扩展事件模型，可以非常轻松地对有效目标上的丢弃做出反应。
...
![Getting started with the droppable widget](graphics/2209OS_11_08.jpg)
...
![Configuring accepted draggables](graphics/2209OS_11_09.jpg)
...
![Configuring drop tolerance](graphics/2209OS_11_10.jpg)
...
![Understanding the droppable callback options](graphics/2209OS_11_12.jpg)
...
DragTables 和DropTables 都具有范围配置选项，允许我们轻松定义拖放对象组和拖放目标。在下一个示例中，我们可以看看如何配置这些选项，以及配置这些选项所产生的效果。在本例中，我们将链接到另一个新样式表，因此在`droppable5.html`的`<head>`元素中，更改`<link>`元素，使其显示如下：
...
将此文件另存为`droppable6.html`。接下来，我们需要创建一个新的 CSS文件；在文本编辑器的新页面中添加以下代码：
...
![Setting the scope options for droppable](graphics/2209OS_11_13.jpg)
...
![Configuring the greedy option](graphics/2209OS_11_14.jpg)
...
![Creating a maze game using the widgets](graphics/2209OS_11_15.jpg)
