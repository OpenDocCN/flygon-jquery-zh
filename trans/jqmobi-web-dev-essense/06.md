...
*   使用示例表单并描述如何处理结果
*   讨论如何构建特定类型的表单以及如何使用 jQueryMobile约定的细节
...
jQueryMobile表单增强的另一个例子是选择菜单。与前面的示例一样，我们使用了一个`fieldcontain div`和`label`标记，但除此之外，`select`标记是正常使用的。以下代码段来自`test5.html:`
...
在本章中，我们讨论了表单以及如何在 jQueryMobile应用中呈现表单。我们讨论了 jQueryMobile 如何自动将所有表单提交转换为基于 Ajax 的调用，并更新表单字段以更好地在移动设备上工作。不仅所有表单字段都会自动更新，还可以使用新控件，如切换、滑块和搜索输入。
