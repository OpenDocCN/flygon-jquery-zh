...
在本章中，我们将研究如何将基于 jQueryMobile的 web 应用转换为移动设备的本机应用。我们将讨论 PhoneGap 框架，以及它如何允许您利用设备的硬件。
...
让我们先看看应用的布局部分，它位于文件的下半部分。您可以看到我们的 jQueryMobile页面结构，其中有一个输入字段、一个按钮和一个空列表。这里的想法是，用户将输入要搜索的名称，点击按钮，结果将显示在列表中。下面的屏幕截图演示了输出：
