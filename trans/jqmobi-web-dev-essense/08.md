# 第 8 章 jQuery 移动配置、实用程序和 JavaScript 方法
...
`Listing 9-4`有点长，但其实很简单。每个`fieldcontain`块由一个特定的路径方法和实用程序测试组成。在模板的下半部分，您可以看到我们使用了`keyup`事件侦听器来监视这些字段的更改并运行每个测试。您可以使用此模板查看这些方法如何根据不同的输入做出反应。下面的屏幕截图显示了一个示例：
