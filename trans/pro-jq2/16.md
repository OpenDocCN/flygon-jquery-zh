...
![image](images/sq.jpg) **提示**我不打算在这一章中试图保留一个可行的非 JavaScript 结构，因为我添加到示例中的所有特性都严重依赖于 JavaScript。
...
![image](images/sq.jpg) **提示**我在脚本中的很多地方动态地插入了元素。我不会让它们保持静态，而是让它们保持原样，这样我就可以专注于添加新功能。
...
![9781430263883_Fig16-01.jpg](images/9781430263883_Fig16-01.jpg)
...
我使用了全局 Ajax 事件来设置一个简单的错误显示。当出现错误时，会创建新元素来描述问题。我展示的简单错误消息来自我从 jQuery 获得的信息，但是在实际的 web 应用程序中，这些消息应该更具描述性，并提供解决方法的建议。你可以在图 16-2 的[中看到一个显示给用户的错误示例。](#Fig2)

![9781430263883_Fig16-02.jpg](images/9781430263883_Fig16-02.jpg)
...
![image](images/sq.jpg) **注意**使用远程验证时需要注意，因为它会给服务器带来很大的负载。在这个例子中，每当用户改变一个`input`元素的值时，我就执行一次远程验证，这在实际应用程序中可能会产生很多请求。更明智的方法通常是在提交表单之前进行远程验证。
...
我禁用了全局事件，因为我不希望在发出远程验证 Ajax 请求时出现的错误被视为用户可以处理的一般错误。相反，我希望它们悄悄地失败，因为当表单被提交时，服务器将执行进一步的验证(正如我在第 13 章中解释的那样，`formserver.js`脚本不执行任何验证，但是真正的 web 应用程序执行验证是很重要的)。
...
![9781430263883_Fig16-03.jpg](images/9781430263883_Fig16-03.jpg)
...
![9781430263883_Fig16-04.jpg](images/9781430263883_Fig16-04.jpg)
...
我不会在真实的 web 应用程序中这样做，但是出于演示的目的，当开发机器和服务器在同一个本地网络上时，这对于强调转换是有用的。你可以在图 16-5 的[中看到 Ajax 请求期间浏览器是如何出现的。](#Fig5)

![9781430263883_Fig16-05.jpg](images/9781430263883_Fig16-05.jpg)
...
![9781430263883_Fig16-06.jpg](images/9781430263883_Fig16-06.jpg)
...
![image](images/sq.jpg) **提示**在前面的例子中，我一直使用`$("form")`选择器，但是由于现在它们在文档中是两种形式，所以我将这些引用切换到使用`form`元素的`id`属性值。
...
![9781430263883_Fig16-07.jpg](images/9781430263883_Fig16-07.jpg)
...
![9781430263883_Fig16-08.jpg](images/9781430263883_Fig16-08.jpg)
