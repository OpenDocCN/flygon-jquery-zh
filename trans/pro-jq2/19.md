...
![image](img/sq.jpg) **提示**我使用 jQuery `map`实用程序方法处理了数组的内容，我在[第 34 章](34.html)中对此进行了描述。
...
![image](img/sq.jpg) **提示**自动完成弹出窗口是使用 jQuery UI *position* 实用程序特性放置的，它比我在本章中使用的配置选项更多。更多详情见`http://api.jqueryui.com/position`。
