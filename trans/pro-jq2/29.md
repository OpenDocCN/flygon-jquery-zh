...
在这个例子中，有两个常规的 jQuery Mobile页面。第一页中的链接就像我在[第 28 章](28.html)中用于导航的链接一样，除了我已经应用了`data-rel`属性并将值设置为`dialog`。
...
通过向对话框的内容添加一个`button`元素，将`data-role`属性设置为 button，将`data-rel`属性设置为`back`，可以向对话框添加一个关闭按钮。我在[第 30 章](30.html)中完整描述了 jQuery Mobile按钮部件，但是你可以在[清单 29-4](#list4) 中看到它是如何应用于对话框部件的。
...
您可以在对话框中添加更多的按钮，这些按钮包含更多的`a`元素，这些元素的`href`属性指向您想要显示的页面。这就是我在[清单 29-5](#list5) 中所做的，我在同一个 HTML 文档中添加了一个到另一个 jQuery Mobile页面的链接。
...
我已经指定将`D`样本用作对话框显示的覆盖图。您可以在[图 29-5](#Fig5) 中看到效果:对话框显示在浅色背景上，而不是与默认`A`样本相关的黑色背景上。(我在[第 28 章](28.html)中描述了主题和样本的 jQuery Mobile方法。)
...
使用 jQuery Mobile弹出窗口小部件

jQuery Mobile弹出窗口小部件在弹出窗口中显示内容。弹出窗口是对话框的轻量级替代，但提供了更多的编程控制。
...
***[清单 29-9](#_list9)*** 。创建 jQuery Mobile弹出窗口小部件
...
[图 29-8](#_Fig8) 。创建 jQuery Mobile弹出窗口
