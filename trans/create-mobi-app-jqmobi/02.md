...
由于 jquerymobile 固有的 Ajax 导航和渐进式增强功能，有很多不同的额外事件。让我们考虑我发现最有用的三个独特的 jQueryMobile事件。我们不会立即使用它们，只是要注意它们，并确保阅读评论。最终，我们将创建`/js/global.js`来容纳我们需要的脚本。现在，只需阅读以下脚本：
